    ...
    <div id="dayReportsSteward">
        ...
        <section class="msg-head clearfix">
            ...
            <div class="fr text-right basic-field-wrapper">
                ...
                <span class="margin-right-5">{{stewardFields.area}}</span>
                <span>统计时间：</span>
                <span>{{stewardFields.time}}</span>
            </div>
        </section>
        <!--基本信息-->
        <section class="basic-area">
            <!--第1行-->
            <div class="basic-list clearfix" v-for="item in turnOneToTwo(stewardFields.basicItem)">
            ...
            <div class="form-wrapper-body table-wrap">
                <div class="table-wrapper table-1-wrapper table-fixed-wrapper">
                    <table cellspacing="0" width="100" class="table-1" cellpadding="0">
                        ...
                        <tbody v-for="(item,index) in stewardFields.formItemFields.formItemArrs" >
                        <tr>
                            <td width="100">
        ...
        <datetime-picker ref="datetimeObj" :isShowSelector="datetimePickerObj.isShow" :transition-type="datetimePickerObj.transitionType"
                beginYear="2015" endYear="2017"  datepickerType="common" v-on:getdate="changeDateSelector">
        </datetime-picker>
    </div>
</template>
...
<script type="text/ecmascript-6">
    ...
    methods:{
        getScheduleData(){
            ...
            self.$http.get("./data1.json")
                    .then((data) =>{
                        ...
                        self.stewardFields.formItemFields.formItemArrs=data.body;
                        console.log(self.stewardFields.formItemFields.formItemArrs);
        ...
        setHeight() {
            //设置固定表格的高度跟滚动表格的高度一致
            $("#dayReportsSteward .table-1 tbody").each(function () {
                var $this=$(this),
                        index=$this.index();
                //获取th
                var $td=$this.find("tr:eq(0) td");
                //获取table-2对应的tbody
                var tbodyHeight=$("#dayReportsSteward .table-2 tbody").eq(index-1).height();
