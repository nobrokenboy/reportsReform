...
gulp.task('brower-sync',function(){
 /*   browerSync.init({
    ...
    browerSync.init({
        proxy: '10.70.7.222:8094/reportForms/dist/views/index.html',
      /*  index: '/reportForms/dist/views/index.html',*/
        port: 8099
    });
});
//编译、合并、压缩、重命名css
...
gulp.task("css",function(){
    ...
    gulp.src(["./develop/static/css/base.scss"])
...
gulp.task("imagemin",function(){
    ...
    /*为什么照片是黑色的*/
    /*gulp.src(['./develop/static/img/!*'])
        .pipe(imageisux('../../../dist/static/img',false));*/
});
//输出字体
...
gulp.task("htmlmin",function(){
    ...
    gulp.src(["./develop/views/**/*.html","./develop/views/**/*.ch"])
        .pipe(changed('./dist/views',{extension:'.html'}))
        .pipe(include({
            includePaths:__dirname+"/develop/views"
        }))
        .pipe(ejs(require("./develop/static/js/config/pattern")).on('error', function (err) {
            this.end();
        }))
...
gulp.task("watch",function(){
    gulp.run(["css","imagemin","font","htmlmin","vue"]);
    gulp.watch("./develop/static/css/**/*.scss",["css"]);
